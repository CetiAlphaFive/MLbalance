...
balance <- function(Y = NULL, W, X, alpha = 0.05, perm.N = 1000, class.method = "ferns", seed = 1995,
  ...
  for (arm in treatment_arms) {
    ...
    # Per-unit arm membership aligned to pscores_real (same idx subset, same order):
    # 0 = control, 1 = this treatment arm. Used by plot.balance() to overlay
    # propensity scores by treatment arm.
    arm_indicator_list[[arm_name]] <- W_binary
